...
public class MergeAudioVideo {
	...
	public static void main(String args[]) {
		/*
		 * if(args.length>1){ System.out.println("Please wait...");
		 */ 
		/*
		 * System.out.println("Merging finished"); } else System.out.println(
		 * "Invalid files input"); System.exit(0);
		 */
	}
	...
	public static void merging(String audioFileName, String videoFileName) {
		...
		try {
			...
			File outFile = new File("mergedvideo.mov");
		...
		try {
			...
			System.exit(-1);
		} catch (NoDataSourceException ie) {
			System.exit(-1);
		}
		// format array for input audio and video
		Format[] formats = new Format[2];
		formats[0] = new AudioFormat(AudioFormat.LINEAR); // create audio format
															// object
		...
		try {
			...
			System.exit(-1);
		} catch (CannotRealizeException ie) {
			ie.printStackTrace();
			System.exit(-1);
		}
