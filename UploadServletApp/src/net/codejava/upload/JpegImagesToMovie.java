...
public class JpegImagesToMovie implements ControllerListener, DataSinkListener {
	...
	public void jpegToMovie(String pngFolder, String outputURL) {
		int width = 1200, height = 800, frameRate = 1;
		Vector<String> inputFiles = new Vector();
		for (File f : new File(pngFolder).listFiles()) {
		 ...
		 * 
		 * // Parse the arguments. int i = 0; int width = -1, height = -1,
		 * frameRate = 1; Vector inputFiles = new Vector(); String outputURL =
		 * null;
		 * 
		 * while (i < args.length) {
		 * 
		 ...
		 * if (i >= args.length) prUsage(); outputURL = args[i]; } else { for
		 * (int j = 0; j < 120; j++) { inputFiles.addElement(args[i]); } } i++;
		 * }
		 * 
		 * if (outputURL == null || inputFiles.size() == 0) prUsage();
		 * 
		 * // Check for output file extension. if (!outputURL.endsWith(".mov")
		 * && !outputURL.endsWith(".MOV")) { System.err.println(
		 * "The output file extension should end with a .mov extension");
		 * prUsage(); }
		 * 
		 * if (width < 0 || height < 0) { System.err.println(
		 * "Please specify the correct image size."); prUsage(); }
		 */
		...
		if (outputURL == null) {
			outputURL = "file:///files/pic" + File.separator + "e.mov";
		}
